...
from tommyskaraoke.lib.dependencies import configure as configure_deps
from tommyskaraoke.lib.ffmpeg import is_ffmpeg_installed
from tommyskaraoke.lib.file_resolver import delete_tmp_dir
from tommyskaraoke.lib.get_platform import get_platform, has_js_runtime
...
async def lifespan(app: FastAPI):
    ...
    # When launched via `uvicorn tommyskaraoke.app_fastapi:combined`, sys.argv contains
    # uvicorn's args (e.g. "--host", "0.0.0.0") which confuse tommyskaraoke's argparse.
    # Keep only recognized tommyskaraoke flags AND their following value tokens.
    _RECOGNIZED = (
    ...
    logging.info(f"TommysKaraoke v{VERSION} ready at http://0.0.0.0:{args.port}")
    logging.info(f"Connect player to: http://0.0.0.0:{args.port}/splash")

    yield  # App is running

    # --- Shutdown ---
    logging.info("Shutting down TommysKaraoke...")
    k.stop()
    delete_tmp_dir()
...
# Leave empty to run at root (classic tommyskaraoke behaviour).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
from tommyskaraoke.routes_fastapi.admin import router as admin_router
from tommyskaraoke.routes_fastapi.background_music import router as bg_music_router
from tommyskaraoke.routes_fastapi.controller import router as controller_router
from tommyskaraoke.routes_fastapi.files import router as files_router
from tommyskaraoke.routes_fastapi.images import router as images_router
from tommyskaraoke.routes_fastapi.info import router as info_router
from tommyskaraoke.routes_fastapi.metadata_api import router as metadata_router
from tommyskaraoke.routes_fastapi.now_playing import router as nowplaying_router
from tommyskaraoke.routes_fastapi.preferences import router as preferences_router
from tommyskaraoke.routes_fastapi.queue import router as queue_router
from tommyskaraoke.routes_fastapi.search import router as search_router
from tommyskaraoke.routes_fastapi.stream import router as stream_router
from tommyskaraoke.routes_fastapi.lyrics import router as lyrics_router
from tommyskaraoke.routes_fastapi.pitch import router as pitch_router
from tommyskaraoke.routes_fastapi.vocal import router as vocal_router
from tommyskaraoke.routes_fastapi.song_config import router as song_config_router
...
def main() -> None:
    args = parse_tommyskaraoke_args()
    uvicorn.run(
        "tommyskaraoke.app_fastapi:combined",
        host="0.0.0.0",
        port=int(args.port),
        log_level="info",
        reload=False,
