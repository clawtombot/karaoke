...
const hasBgVideo = TommysKaraokeConfig.hasBgVideo;
let currentVideoUrl = null;
let hlsInstance = null;
let idleTime = 0;
let screensaverTimeoutSeconds = TommysKaraokeConfig.screensaverTimeout;
let bg_playlist = [];
...
const endSong = async (reason = null, showScore = false) => {
  ...
  teardownStemMixer();
  if (showScore && !TommysKaraokeConfig.disableScore) {
    isScoreShown = true;
...
const playBGMusic = async (play) => {
  ...
  if (play) {
    if (TommysKaraokeConfig.disableBgMusic) return;
    ...
    $(audio).animate({ volume: TommysKaraokeConfig.bgMusicVolume }, 2000);
  } else {
    if (audio) {
...
const playBGVideo = async (play) => {
  ...
  if (play) {
    if (TommysKaraokeConfig.disableBgVideo) return;
    if (!autoplayConfirmed) return;
...
const handleNowPlayingUpdate = (np) => {
  ...
  if (np.boot_id && TommysKaraokeConfig.bootId && np.boot_id !== TommysKaraokeConfig.bootId) {
    console.log("Server restarted (boot_id:", TommysKaraokeConfig.bootId, "->", np.boot_id, ") — reloading");
...
const setupOverlayMenus = () => {
  if (TommysKaraokeConfig.hideOverlay) {
  ...
  const triggerInactivity = () => {
    ...
    cursorVisible = false;
    $("#menu a").fadeOut();
    if (TommysKaraokeConfig.showSplashClock) {
      setTimeout(() => {
...
const handleUnsupportedBrowser = () => {
  if (!isSupportedBrowser) {
    ...
    warningMessage.innerHTML =
      TommysKaraokeConfig.translations.unsupportedBrowser;
...
const toggleBGMedia = (configKey, playFn, disabled) => {
  TommysKaraokeConfig[configKey] = disabled;
...
const PREFERENCE_EFFECTS = {
  ...
  disable_score:       (v) => { TommysKaraokeConfig.disableScore = v; },
  show_splash_clock:   (v) => {
    TommysKaraokeConfig.showSplashClock = v;
    v ? startClock() : (stopClock(), $("#clock").hide());
  },
  hide_overlay:        (v) => {
    TommysKaraokeConfig.hideOverlay = v;
    $("#bottom-container, #top-container").toggle(!v);
  },
  hide_url:            (v) => { $("#qr-code, #screensaver-qr").toggle(!v); },
  bg_music_volume:     (v) => {
    TommysKaraokeConfig.bgMusicVolume = v;
  ...
  screensaver_timeout: (v) => {
    ...
    TommysKaraokeConfig.screensaverTimeout = v;
  },
};
...
const setupSocketEvents = () => {
  ...
  socket.on('connect_error', (error) => {
    ...
    flashNotification(TommysKaraokeConfig.translations.socketConnectionLost, "is-danger");
  });
...
$(function () {
  ...
  setupUIScaling();
  if (TommysKaraokeConfig.showSplashClock) startClock();
  setupScreensaver();
  setupOverlayMenus();
  setupVideoPlayer();
