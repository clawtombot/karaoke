...
from tommyskaraoke.lib.dependencies import get_karaoke, get_sio
from tommyskaraoke.lib.lyrics import LyricsManager
from tommyskaraoke.lib.lyrics.cache import LyricsCache
...
async def lyrics_candidates(
    ...
    import httpx
    from tommyskaraoke.lib.lyrics import lrclib, netease
...
async def select_lyrics(body: SelectLyricsBody):
    ...
    if source == "netease":
        from tommyskaraoke.lib.lyrics import netease
        lyrics = await netease.get_lyrics(int(sid))
    elif source == "lrclib":
        import httpx
        from tommyskaraoke.lib.lyrics import lrclib
        async with httpx.AsyncClient(timeout=10.0, headers={"User-Agent": lrclib.USER_AGENT}) as client:
...
def _parse_filename(filename: str) -> tuple[str, str]:
    ...
    Handles TommysKaraoke format (Title---ID.mp4) and yt-dlp format (Title [ID].mp4).
    Attempts to split on common separators like " - " for artist/title.
    """
    # Strip extension
    name, _ = os.path.splitext(filename)

    # Remove video ID: TommysKaraoke "---XXXXXXXXXXX" or yt-dlp " [XXXXXXXXXXX]"
    if "---" in name:
