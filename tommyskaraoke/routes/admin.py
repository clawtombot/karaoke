...
from tommyskaraoke.lib.current_app import get_admin_password, get_karaoke_instance, is_admin
from tommyskaraoke.lib.youtube_dl import get_youtubedl_version, upgrade_youtubedl
...
def quit():
    ...
    k = get_karaoke_instance()
    if is_admin():
        # MSG: Message shown after quitting tommyskaraoke.
        msg = _("Exiting tommyskaraoke now!")
        flash(msg, "is-danger")
        k.send_notification(msg, "danger")
        th = threading.Thread(target=delayed_halt, args=[0, k])
        th.start()
    else:
        # MSG: Message shown after trying to quit tommyskaraoke without admin permissions.
