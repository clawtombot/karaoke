"""Command-line argument parsing for TommysKaraoke."""

import argparse
import logging
import os

from tommyskaraoke.lib.get_platform import get_default_dl_dir, get_platform
from tommyskaraoke.lib.preference_manager import PreferenceManager
from tommyskaraoke.lib.separation import env_or_none, parse_bool_env
...
def parse_tommyskaraoke_args() -> argparse.Namespace:
    """Parse command-line arguments for TommysKaraoke.
    ...
    parser.add_argument(
        ...
        "--headless",
        action="store_true",
        help="Headless mode. Don't launch the splash screen/player on the tommyskaraoke server",
        required=False,
    )
    parser.add_argument(
        "--logo-path",
    ...
    parser.add_argument(
        "--hide-overlay",
        action="store_true",
        help="Hide all overlays that show on top of video, including current/next song, tommyskaraoke QR code and IP",
        required=False,
    )
    parser.add_argument(
        "--hide-notifications",
    ...
    parser.add_argument(
        "--limit-user-songs-by",
        help=f"Limit the number of songs a user can add to queue. User name 'TommysKaraoke' is always unlimited (default: {_DEFAULTS['limit_user_songs_by']} = unlimited)",
        default=None,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help="Enable configurable 6-stem separation. Requires explicit backend and model config.",
    )
    parser.add_argument(
        "--separation-backend",
        choices=["demucs", "mlx"],
        default=None,
        help="Stem separation backend. Required when --vocal-splitter is enabled.",
    )
    parser.add_argument(
        "--separation-device",
        choices=["cpu", "cuda", "mps", "mlx"],
        default=None,
        help="Device for the separation backend. Required for demucs; optional for mlx.",
    )
    parser.add_argument(
        "--separation-model",
        default=None,
        help="First-pass separation model name. Required when --vocal-splitter is enabled.",
    )
    parser.add_argument(
        "--vocal-split-model",
        default=None,
        help="Second-pass lead/backing vocal split model. Required when --vocal-splitter is enabled.",
    )
    parser.add_argument(
        "--model-cache-dir",
        default=None,
        help="Directory used for downloaded model caches. Keeps weights out of the repo.",
    )

    args = parser.parse_args()

    if args.vocal_splitter is None:
        args.vocal_splitter = parse_bool_env("TOMMYSKARAOKE_VOCAL_SPLITTER") or False

    args.separation_backend = args.separation_backend or env_or_none("TOMMYSKARAOKE_SEPARATION_BACKEND")
    args.separation_device = args.separation_device or env_or_none("TOMMYSKARAOKE_SEPARATION_DEVICE")
    args.separation_model = args.separation_model or env_or_none("TOMMYSKARAOKE_SEPARATION_MODEL")
    args.vocal_split_model = args.vocal_split_model or env_or_none("TOMMYSKARAOKE_VOCAL_SPLIT_MODEL")
    args.model_cache_dir = args.model_cache_dir or env_or_none("TOMMYSKARAOKE_MODEL_CACHE_DIR")
    ...
    args.model_cache_dir = arg_path_parse(args.model_cache_dir)
