"""Platform detection utilities for TommysKaraoke."""
...
def get_default_dl_dir(platform: str) -> str:
    ...
    """
    if is_raspberry_pi():
        return "~/tommyskaraoke-songs"
    elif is_windows():
        legacy_directory = os.path.expanduser("~\\tommyskaraoke\\songs")
        if os.path.exists(legacy_directory):
            return legacy_directory
        else:
            return "~\\tommyskaraoke-songs"
    else:
        legacy_directory = "~/tommyskaraoke/songs"
        if os.path.exists(legacy_directory):
            return legacy_directory
        else:
            return "~/tommyskaraoke-songs"
...
def get_data_directory() -> str:
    ...
    """
    if is_windows():
        # Windows: %APPDATA%/tommyskaraoke
        base_path = os.environ.get("APPDATA")
        # Fallback if APPDATA is not set (rare, but possible)
        if not base_path:
            base_path = os.path.expanduser("~")
        path = os.path.join(base_path, "tommyskaraoke")
    else:
        # Linux, macOS, Android, Raspberry Pi: ~/.tommyskaraoke
        path = os.path.expanduser("~/.tommyskaraoke")
