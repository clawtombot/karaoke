...
class TestHasJsRuntime:
    ...
    def test_has_runtime(self):
        ...
        with patch("tommyskaraoke.lib.get_platform.get_installed_js_runtime", return_value="node"):
            assert has_js_runtime() is True

    def test_no_runtime(self):
        """Test when no JS runtime is available."""
        with patch("tommyskaraoke.lib.get_platform.get_installed_js_runtime", return_value=None):
...
class TestGetPlatform:
    ...
    def test_osx_platform(self):
        ...
        with patch("sys.platform", "darwin"):
            with patch("tommyskaraoke.lib.get_platform.is_android", return_value=False):
                with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_windows_platform(self):
        ...
        with patch("sys.platform", "win32"):
            with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=True):
                with patch("tommyskaraoke.lib.get_platform.is_android", return_value=False):
                    with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_linux_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=False):
                with patch("tommyskaraoke.lib.get_platform.is_android", return_value=False):
                    with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_android_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("tommyskaraoke.lib.get_platform.is_android", return_value=True):
    ...
    def test_raspberry_pi_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("tommyskaraoke.lib.get_platform.is_android", return_value=False):
                with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=True):
...
class TestGetDefaultDlDir:
    ...
    def test_raspberry_pi_default(self):
        ...
        with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=True):
            ...
            assert result == "~/tommyskaraoke-songs"
        ...
        with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=False):
            with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=True):
                with patch("os.path.exists", return_value=False):
                    ...
                    assert result == "~\\tommyskaraoke-songs"
        ...
        with patch("tommyskaraoke.lib.get_platform.is_raspberry_pi", return_value=False):
            with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=True):
                with patch("os.path.exists", return_value=True):
                    with patch(
                        "os.path.expanduser", return_value="C:\\Users\\test\\tommyskaraoke\\songs"
                    ):
                        result = get_default_dl_dir("windows")
                        assert "tommyskaraoke\\songs" in result
    ...
    def test_windows_path(self):
        ...
        with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=True):
            with patch.dict(os.environ, {"APPDATA": "C:\\Users\\Test\\AppData\\Roaming"}):
                ...
                with patch("tommyskaraoke.lib.get_platform.os.path") as mock_path:
                    ...
                    assert result == "C:\\Users\\Test\\AppData\\Roaming\\tommyskaraoke"
            ...
            with patch.dict(os.environ, {"APPDATA": "C:\\Users\\Test\\AppData\\Roaming"}):
                with patch("os.makedirs") as mock_makedirs:
                    ...
                    with patch("tommyskaraoke.lib.get_platform.os.path") as mock_path:
                        ...
                        expected_path = "C:\\Users\\Test\\AppData\\Roaming\\tommyskaraoke"
                        mock_makedirs.assert_called_once_with(expected_path)

    def test_linux_path(self):
        """Test that Linux/Mac returns the home directory path."""
        with patch("tommyskaraoke.lib.get_platform.is_windows", return_value=False):
            with patch("os.path.expanduser", return_value="/home/test/.tommyskaraoke"):
                with patch("os.path.exists", return_value=True):
                    result = get_data_directory()
                    assert result == "/home/test/.tommyskaraoke"
